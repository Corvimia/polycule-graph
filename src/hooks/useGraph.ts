...
export function useGraph() {
  ...
  const addNode = useCallback(
    (options?: { id?: string; label?: string; position?: { x: number; y: number } }) => {
      let id = options?.id
      if (!id) {
  ...
  const renameNode = useCallback(
    (id: string, nextId: string, label: string) => {
      ...
      setSelected(prev =>
        prev && prev.type === 'node' && prev.id === id ? { ...prev, id: nextId } : prev,
      )
    },
    [nodes],
  )
      ...
      eds.map(e =>
        e.id === id
          ? {
              ...
              style: {
                ...e.style,
                stroke: data.color ?? e.data.color,
                strokeWidth: data.width ?? e.data.width,
              },
            }
          : e,
      ),
