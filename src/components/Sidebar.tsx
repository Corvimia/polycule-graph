          ...
          <Dialog.Content className="fixed left-0 top-0 h-full w-4/5 max-w-[320px] bg-white dark:bg-neutral-900 shadow-2xl z-40 flex flex-col">
            ...
            <div className="px-5 pt-8 pb-2 border-b border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800 flex flex-col items-center gap-2 text-left relative">
              <div className="flex items-center gap-2">
                ...
                <span className={dark ? 'font-bold text-xl text-white' : 'font-bold text-xl text-neutral-900'}>polycule graph</span>
              </div>
              <span className={dark ? 'font-medium text-xs text-indigo-300' : 'font-medium text-xs text-indigo-400'}>v{pkg.version}</span>
              {/*
              <button
                onClick={toggle}
            ...
            <div className="flex-1 overflow-y-auto px-5 pt-5 pb-4 flex flex-col">
              <div className={dark ? 'font-semibold text-indigo-300 mb-2 text-sm tracking-wide' : 'font-semibold text-indigo-600 mb-2 text-sm tracking-wide'}>DOT Markup</div>
              <div style={{ border: `2px solid ${(dotDraftError ? '#e11d48' : (dark ? '#23272f' : '#23272f'))}`, borderRadius: 8 }}>
                <MonacoEditor
                  height="260px"
    ...
    <aside className="w-80 bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 flex flex-col h-full shadow-lg z-20 fixed left-0 top-0">
      <div className="px-5 pt-8 pb-2 border-b border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800 flex flex-col items-center gap-2 text-left relative">
        <div className="flex items-center gap-2">
          ...
          <span className={dark ? 'font-extrabold text-2xl text-white tracking-tight drop-shadow' : 'font-extrabold text-2xl text-neutral-900 tracking-tight'}>polycule graph</span>
        </div>
        <span className={dark ? 'font-medium text-xs text-indigo-300' : 'font-medium text-xs text-indigo-400'}>{pkg.version}</span>
        {/*
        <button
          onClick={toggle}
          className="absolute right-2 top-2 p-2 rounded-full bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600 transition-colors"
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-6 pb-4 flex flex-col">
        <div className={dark ? 'font-semibold text-indigo-300 mb-2 text-sm tracking-wide uppercase' : 'font-semibold text-indigo-600 mb-2 text-sm tracking-wide uppercase'}>DOT Markup</div>
        <div style={{ border: `2px solid ${(dotDraftError ? '#e11d48' : (dark ? '#23272f' : '#23272f'))}`, borderRadius: 12 }}>
          <MonacoEditor
            height="260px"
            defaultLanguage="plaintext"
            theme={dark ? 'vs-dark' : 'light'}
